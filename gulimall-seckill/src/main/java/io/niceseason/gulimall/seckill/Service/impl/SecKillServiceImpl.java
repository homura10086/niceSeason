...
//    存储模型设计
//    秒杀场次存储的List可以当做hash key在SECKILL_CHARE_PREFIX 中获得对应的商品数据

    //存储的秒杀场次对应数据
    //K: SESSION_CACHE_PREFIX + startTime + "_" + endTime
    //V: string，sessionId+"-"+skuId的List
        ...
        if (!StringUtils.isEmpty(json)){
            ...
            if (current >= redisTo.getStartTime() && current <= redisTo.getEndTime()) {
                ...
                if (redisKey.equals(killId) && redisTo.getRandomCode().equals(key)) {
                    ...
                    Boolean occupy = redisTemplate.opsForValue().setIfAbsent(
                            memberResponseVo.getId() + "-" + redisTo.getSkuId(), num.toString(), ttl,
                            TimeUnit.MILLISECONDS);
                    //3.2 占位成功，说明该用户未秒杀过该商品，则继续
                    if (Boolean.TRUE.equals(occupy)){
                        //4. 校验库存和购买量是否符合要求
                        if (num <= redisTo.getSeckillLimit()) {
                            //4.1 尝试获取库存信号量
                            RSemaphore semaphore = redissonClient.getSemaphore(
                                    SKU_STOCK_SEMAPHORE + redisTo.getRandomCode());
...
//    在redis中保存秒杀商品信息
    private void saveSecKillSku(List<SeckillSessionWithSkusVo> sessions) {
        BoundHashOperations<String, Object, Object> ops = redisTemplate.boundHashOps(SECKILL_CHARE_PREFIX);
        sessions.forEach(session -> session.getRelations().forEach(sku -> {
